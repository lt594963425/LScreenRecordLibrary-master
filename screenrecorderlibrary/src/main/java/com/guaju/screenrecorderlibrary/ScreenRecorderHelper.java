...
public class ScreenRecorderHelper {
  ...
  boolean  sAudio;
    ...
    public void startCustomRecord(final Activity activity, final int width, final int height,String dirpath,String fileName,boolean audio ) {
        MediaProjectionManager projectionManager = (MediaProjectionManager) mcontext.getSystemService(Context.MEDIA_PROJECTION_SERVICE);
        recordService.setRecorderConfig(width, height,dirpath,fileName,sAudio);
        if (recordService.isRunning()) {
            recordService.stopRecord();
        } else {
            Intent captureIntent = projectionManager.createScreenCaptureIntent();
    ...
    public String getsaveDirectory(String dirPath) {
        String rootDir;
